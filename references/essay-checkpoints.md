...
write 문항에는 다음 형태를 사용한다. 예시는 가상 사실이며 실제 문서에는 확인된 자료로 바꾼다. 상세 판단 기준은 [작성 지침](essay-workflow.md)의 문항 분석, 정보 선택과 본문만으로 읽는 검토를 따른다.
...
valid 답변에는 다음처럼 본문에서 읽히는 내용과 문항 적합성 근거를 기록한다. summary의 필수 필드는 writing_brief와 같다. 설계의 의도를 복사하지 않는다.
